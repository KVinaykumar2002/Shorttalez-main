-- Create debug system tables with different enum names to avoid conflicts
CREATE TYPE public.debug_log_level AS ENUM ('debug', 'info', 'warn', 'error', 'critical');
CREATE TYPE public.debug_log_status AS ENUM ('new', 'reviewing', 'fixing', 'fixed', 'ignored');

-- Debug logs table to store all application logs
CREATE TABLE public.debug_logs (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  level debug_log_level NOT NULL DEFAULT 'info',
  ...
  status debug_log_status DEFAULT 'new',
...
CREATE TABLE public.debug_settings (
  ...
  allowed_log_levels debug_log_level[] DEFAULT ARRAY['error', 'critical'],
  created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(),
  updated_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now()
);
...
INSERT INTO public.debug_settings (debug_mode_enabled, auto_fix_enabled)
VALUES (false, false);
